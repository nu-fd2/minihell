...
/*   Updated: 2025/07/09 22:05:23 by oel-mado         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*rdd(char *buff, int fd)
{
	...
	yup = ftg_calloc(BUFFER_SIZE + 1, 1);
	if (!yup)
		return (free(buff), buff = NULL, NULL);
	i = 1;
	while (!ftg_strchr(yup, '\n') && i != 0)
	{
...
char	*updtt(char *buff)
{
	...
	if (buff[i] == '\0')
		return (free(buff), buff = NULL, NULL);
	j = ftg_strlen(buff + i + 1);
	if (j <= 0)
		return (free(buff), buff = NULL, NULL);
	neo = ftg_calloc(j + 1, 1);
	if (!neo)
		return (free(buff), buff = NULL, NULL);
	ftg_strlcpy(neo, buff + i + 1, j + 1);
